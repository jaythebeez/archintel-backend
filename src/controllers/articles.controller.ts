...
import { ArticleFormData, validateArticle } from "../utils/validation/validateArticle.js";
import { parseArticleData } from "../utils/parseData/parseArticleData.js";
import UserModel from "../models/user.model.js";

export const getAllArticles = async (req: AuthenticatedRequest, res:Response, next: NextFunction) => {
    try{
...
export const addArticle = async (req: AuthenticatedRequest, res:Response, next: NextFunction) => {
    try{
        const userId = req.userId;
...
export const editArticle = async (req: AuthenticatedRequest, res:Response, next: NextFunction) => {
    try{
        const userId = req.userId;
...
export const publishArticle = async (req: AuthenticatedRequest, res:Response, next: NextFunction) => {
    try{
        const userId = req.userId;
        const articleId = req.params.articleId;
        ...
        if(user.type !== "Editor") res.status(400).json("You must be an editor to publish");
